...
CMD_FRMT = "UPDATE \"API_specific\" SET expert = \'%s\' WHERE general = \'%s\';"
...
def create_cmd_str( cmd_metalist ):
    ...
    while header_index < len( header_list ):
        if header_list[header_index] == "API":
            general_index = header_index

        elif header_list[header_index] == "Expert-Fabio":
    ...
    for list in cmd_metalist[1:]:
        ...
        if expert_item.lower() not in { "lixo", "trash" }:
            cmd_str = CMD_FRMT %( expert_item, general_item )
